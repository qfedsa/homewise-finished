...
import { FileText, Plus, ToggleLeft, ToggleRight, Edit, Trash2 } from 'lucide-react';
...
export const AutomationRulesPage: React.FC = () => {
  ...
  const [editedRule, setEditedRule] = useState<Omit<AutomationRule, 'id'>>({
    name: '',
    interval: 'Jährlich',
    status: 'Aktiv',
  });
  ...
  const handleSaveEdit = () => {
    setAutomationRules(prevRules =>
      prevRules.map(rule =>
        rule.id === editingRuleId ? { ...rule, ...editedRule } : rule
      )
    );
  ...
  const handleDeleteRule = (ruleId: string) => {
    setAutomationRules(prevRules => prevRules.filter(rule => rule.id !== ruleId));
  };
                ...
                <td className="px-5 py-5 border-b border-gray-200 text-sm">
                  {editingRuleId === rule.id ? (
                    ...
                    <>
                      ...
                      >
                        <Edit className="w-4 h-4 inline-block align-middle" /> Bearbeiten
                      </button>
                      <button
